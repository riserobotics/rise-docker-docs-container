...
# ------------------------------------------------------------
# System packages + Go (for Hugo modules); do NOT install hugo via apt
# ------------------------------------------------------------
RUN apt-get update && \
    apt-get install -y --no-install-recommends \
      ca-certificates curl wget git unzip \
      tini dumb-init gnupg tar xz-utils \
      adduser sudo procps iproute2 psmisc \
      python3 python3-pip python3-venv \
      golang-go && \
    rm -rf /var/lib/apt/lists/*

# ------------------------------------------------------------
# Install Hugo Extended (>= 0.134.0) from GitHub release
# ------------------------------------------------------------
...
# ------------------------------------------------------------
# Flask clone-repo app (serves on 8081 → setup.localhost) with venv
# ------------------------------------------------------------
...
# ------------------------------------------------------------
# Startup script: Traefik (80), code-server (8080), Flask app (8081)
# ------------------------------------------------------------
